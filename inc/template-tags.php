...
if ( ! function_exists( 'tsumugi_posted_on' ) ) :
/**
 * Prints HTML with meta information for the current post-date/time and author.
 */
function tsumugi_posted_on() {
...
if ( ! function_exists( 'tsumugi_entry_footer' ) ) :
/**
 * Prints HTML with meta information for the categories, tags and comments.
 */
function tsumugi_entry_footer() {
 ...
 * @return bool
 */
function tsumugi_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'tsumugi_categories' ) ) ) {
		...
		set_transient( 'tsumugi_categories', $all_the_cool_cats );
	}

	if ( $all_the_cool_cats > 1 ) {
		// This blog has more than 1 category so tsumugi_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so tsumugi_categorized_blog should return false.
		return false;
	}
}

/**
 * Flush out the transients used in tsumugi_categorized_blog.
 */
function tsumugi_category_transient_flusher() {
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
		return;
	}
	// Like, beat it. Dig?
	delete_transient( 'tsumugi_categories' );
}
add_action( 'edit_category', 'tsumugi_category_transient_flusher' );
add_action( 'save_post',     'tsumugi_category_transient_flusher' );
